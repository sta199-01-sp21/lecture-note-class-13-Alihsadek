...
You're not getting the exact value but rather a range of different values that
the true value is likely to be found in. 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/Lecture_13_Notes/manhattan.csv")
```
...
- Sample: Sample of 1-bedroom apartments
- Sample size: 20
...
What does this mean?
This means that 95% of the time, we are confident that the true value will
appear in the sample. 95% confidence level is the standard used in research
today. 
...
Consider the mice radiation dataset. Suppose we want to compute a 95% confidence interval for the proportion of mice that died while on the treatment,regardless of the dose.
...
What does this mean?
We are 95% confidence that the true proportion is in this interval.
...
We can confirm at the 95% confidence level that the proportion of mice that
lived (while on the treatment) was between 0.337 and 0.460.
